 ...
 * @param   string
 * @param   int
 * @param   string the end of character. Ussually on ellipsis
 * @return  string
 */
...
function slugify($text)
{
    ...
    // vše co nejsou písmena nahraď pomlčkami
    $text = preg_replace('~[^\\pL\d]+~u', '-', $text);

    // trim
    // nadbytečné se odstraní
    $text = trim($text, '-');

    // transliterate
    // symboly typu šžčě se změní na szce
    $text = iconv('utf-8', 'us-ascii//TRANSLIT', $text);

    // lowercase
    // vše se hodí na lowercase
    $text = strtolower($text);

    // remove unwanted characters
    // prázdné mezery a tabulátory se vymažou
